    ...
    if (pins?.length)
        view.pins = pins
            .map((p, i) => {
                ...
                const c = po && classifyPattern(po.path);
                if (!c) return null;
                if (c.error) return void r.issues.add(`${path}.pins[${i}].path`, c.error, c.message);
                return { path: c.pattern.path ?? c.pattern.glob };
            })
            .filter(Boolean);
    ...
    if (mons?.length)
        view.monitors = mons
            .map((m, i) => {
                ...
                if (!mo) return null;
                const c = classifyPattern(mo.path);
                if (c.error) return void r.issues.add(`${path}.monitors[${i}].path`, c.error, c.message);
                const mode = r.enumOf(mo.mode ?? "diff-feed", `${path}.monitors[${i}].mode`, ["diff-feed", "files"]);
                return { path: c.pattern.path ?? c.pattern.glob, title: r.str(mo.title, `${path}.monitors[${i}].title`, { required: false, max: 80 }), mode };
            })
            .filter(Boolean);
...
export function parsePlan(issues, input, ctx) {
    ...
    phases.forEach((pv, pi) => {
        ...
        const phase = { id, title: r.str(po.title, `${pp}.title`, { max: 160 }), expects: parseExpects(r, po.expects, `${pp}.expects`, ctx, counter), steps: [], state: prevPhase.get(id)?.state ?? { status: "pending" } };
