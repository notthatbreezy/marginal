...
export async function commandFront(input, ctx) {
    ...
    const next = writeState(docId, (s) => {
        ...
        // Referential integrity: a re-registered id must not inherit the removed front's plan associations.
        for (const ph of s.plan?.phases ?? []) {
            if (ph.state.status === "active") ph.state.frontIds = ph.state.frontIds.filter((f) => f !== id);
            for (const st of ph.steps) if (st.state.status === "active" && st.state.frontId === id) delete st.state.frontId;
        }
    });
...
export const ACTIONS = { command_plan: commandPlan, command_front: commandFront, command_status: commandStatus, command_read: commandRead, command_view: commandView };
