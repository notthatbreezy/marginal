// M3: checkpoint refs → commit pairs, command_diff (authoring aid), command_walkthrough (show / edit / close).
 ...
 */
export async function resolveRef(r, v, path, { state, repo, docId, persistAs } = {}) {
    ...
    if (kind === "live") {
        ...
        try {
            const snap = await snapshotWorktree(front.worktree, { message: `whiteboard live snapshot ${frontId}`, ref: persistAs ? refFor(docId, persistAs) : undefined });
...
async function parseStop(r, v, path, diff, { partial = false } = {}) {
    ...
    if (!partial || o.ranges !== undefined) {
        ...
        for (const [i, rv] of ranges.entries()) {
            ...
            stop.ranges.push({ file: n.path, side, startLine: start, endLine: end });
        }
    }
...
export async function commandWalkthrough(input, ctx) {
    ...
    if (op === "show") {
        ...
        const from = await resolveRef(r, o.from, "walkthrough.from", { ...env, persistAs: id && `walk-${id}-from` });
        const to = await resolveRef(r, o.to, "walkthrough.to", { ...env, persistAs: id && `walk-${id}-to` });
        ...
        const prev = state.walkthroughs.find((w) => w.id === id);
        ...
        const next = writeState(docId, (s) => {
            ...
            s.walkthroughView = { id, stopId: w.stops[0].id, seq: (s.walkthroughView?.seq ?? 0) + 1 };
        });
    ...
    const next = writeState(docId, (s) => {
        ...
        if (focusId) s.walkthroughView = { id, stopId: focusId, seq: (s.walkthroughView?.seq ?? 0) + 1 };
        else if (s.walkthroughView?.id !== id) s.walkthroughView = { id, stopId: s.walkthroughView?.stopId && stops.some((x) => x.id === s.walkthroughView.stopId) ? s.walkthroughView.stopId : stops[0].id, seq: (s.walkthroughView?.seq ?? 0) + 1 };
    });
    revising(docId, id, false);
