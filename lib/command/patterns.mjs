...
export function offPlanMatcher(plan, frontId) {
    ...
    for (const ph of plan.phases) {
        // Work this front did for finished checkpoints stays in-plan; only active and done phases carry frontIds.
        if (ph.state.status !== "pending" && ph.state.frontIds?.includes(frontId)) assoc.push(...phasePatterns(ph));
...
export function parsePrefs(v) {
    ...
    if (o.guideSeen === true) out.guideSeen = true;
