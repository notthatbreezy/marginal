...
export function totalsFromLog(docId, frontId, sinceSeq = 0) {
    const m = new Map();
    for (const e of eventsSince(docId, sinceSeq)) if (e.frontId === frontId) m.set(e.file, isPresentChange(e) ? { add: e.totals.add, del: e.totals.del, kind: e.kind, binary: e.binary, sig: e.sig } : null);
...
export async function pollOnce(loop) {
    ...
    const since = front.sinceSeq ?? 0; // this registration's incarnation
