...
export async function startServer({ chat, instances, getSessionId }) {
    ...
    async function commandRoute(req, res, url, sub) {
        ...
        if (what === "init" && req.method === "POST") {
            ...
            const lease = readLease(doc.id);
            if (lease?.live && lease.sessionId !== getSessionId?.()) throw new InputError(`Session ${lease.sessionId} already orchestrates this whiteboard; open it there.`);
            ...
            const lines = [
                ...
                goal ? `The user's goal: ${goal}` : "The user didn't give a goal; infer it from this whiteboard, the branch and our conversation.",
                [
                    "Set up the Command center for the implementation work this whiteboard is about, so its Command tab can track it live:",
                    '1. Read instructions {topic:"command"}.',
                    `2. Work out the plan from the whiteboard content, the change ${t.base.slice(0, 8)}…${t.head.slice(0, 8)} in ${repo.name} (${repo.path}) and anything we've already discussed. Checkpoints (phases) → steps → expects (prefer directories and globs).`,
    ...
    async function askCommand({ doc, instanceId, quote, message, threadId, focus }) {
        const lease = readLease(doc.id);
        if (lease?.live && lease.sessionId !== getSessionId?.()) throw new InputError(`Command chat talks to the orchestrator (session ${lease.sessionId}): open this whiteboard in that session.`);
...
/** Focus payload (spec §7.10) from the panel: keep only well-formed items. */
