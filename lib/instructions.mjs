...
const common = `All calls are invoke_canvas_action on an open "whiteboard" canvas instance. documentId defaults to whatever that panel is showing.
...
export const instructions = {
    authoring: `You are writing an interactive, RFC-style whiteboard that a staff engineer will read to understand a change.

${common}

**Flow** — do these first steps without extra detours:
1. register_repository {path} → repositoryId.
2. create {title, target: {repositoryId, base, head}} (base/head can be refs such as "origin/main" and "HEAD"; they resolve to the merge-base and head commits), or create {pullRequestUrl, repositoryId}. The panel switches to the new whiteboard.
...
**Updating an existing whiteboard**: set_target to the new commits (it reports sources that no longer resolve), read the diff since the old head, and update the affected blocks.

${blocks}`,

    scratchpad: `The scratchpad (documentId "scratchpad") is a single always-present whiteboard with no target — use it to sketch an explanation the way you would on a real whiteboard: a short paragraph, a sequence or flow diagram, a call tree, a code peek.

${common}

**When**: the user asks to be shown how something works, wants a picture of a flow / call path / data shape, is thinking out loud, or the explanation spans several files or repos (anything you'd otherwise draw as ASCII art). Answer in chat when a sentence does it. Don't review a change here — create a whiteboard instead.
...
${blocks}`,
    ...
    "file-lenses": `Group a whiteboard's changed files into file lenses for the Diff tab.
...
5. lens {op:"insert", title, paths:[files or "dir/" prefixes], collapsed?} — each response lists what is still uncategorized. Finish with nothing uncategorized, then activity {action:"end", scope:"lenses"}.`,
    ...
    command: `**Command center protocol.** The Command tab of a whiteboard with a repository target is a live mission wall for a multi-step implementation: a territory map of the repo lights up as the worktrees you (and your child sessions) edit change. When implementing a multi-step plan:
1. command_plan {op:"set", plan:{id, title, base?, phases:[{id, title, expects:[paths, "dir/" or globs], steps?:[{id, title, expects}], suggestedView?}]}} once. Prefer directories and globs over exhaustive file lists. This claims the whiteboard's Command lease for YOUR session: only this session may change Command state afterwards, and the Command chat talks to it.
