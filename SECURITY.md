...
- The extension reads your repositories with `git` and stores its data locally in `artifacts/` (or `WHITEBOARD_DATA_DIR`). It makes no network requests of its own.
- Writes to your repositories are limited to hidden checkpoint refs under `refs/whiteboard/checkpoints/`. Branches, HEAD, the index and working trees are never modified.
