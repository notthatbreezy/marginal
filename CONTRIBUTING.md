...
| Path | What |
|---|---|
| `extension.mjs` | Joins the Copilot session, declares the `whiteboard` canvas and its actions. |
| `lib/` | Node side: git access (`git.mjs`), whiteboard storage (`store.mjs`), block schemas (`blocks.mjs`), the loopback server (`server.mjs`), side chat (`chat.mjs`), agent guidance (`instructions.mjs`). |
...
| `test/` | `node:test` suites and a smoke test. They create throwaway git repos and set `WHITEBOARD_DATA_DIR` to a temp dir. |
| `tools/` | `devserver.mjs` (standalone Command tab with a fake repo) and `scope-css.mjs`. |

## Workflow

1. Clone into `~/.copilot/extensions/whiteboard` (or symlink your checkout there).
