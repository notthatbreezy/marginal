...
export const refFor = (docId, name) => `refs/marginal/checkpoints/${docId}/${name}`;
...
export async function snapshotWorktree(worktree, { message, ref } = {}) {
    ...
    try {
        ...
        const sha = (await gitOut(worktree, ["-c", "user.name=Marginal", "-c", "user.email=marginal@localhost", "commit-tree", tree, "-p", head, "-m", message ?? "marginal checkpoint"], { kind: "commit-tree" }))?.trim();
...
/** Remove every checkpoint ref of a document (called when the doc is deleted). */
export async function dropRefs(repoPath, docId) {
    const out = await gitOut(repoPath, ["for-each-ref", "--format=%(refname)", `refs/marginal/checkpoints/${docId}/`, `refs/whiteboard/checkpoints/${docId}/`], { kind: "for-each-ref" }); // + pre-rename refs
