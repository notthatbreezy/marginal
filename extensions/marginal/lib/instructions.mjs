...
- Tour notes: steps, flow nodes and call-stack frames each take notes?: [{title?, text? (markdown), source? | code?{language,text}}] (max 12). The diagram's Tour/Focus view shows them under the stop as "Notes & examples". Leave them out when drawing; add them when the reader asks for more on a stop ("show an example", "what does this look like?") with edit {type:"update", targetId:<step/node/frame id>, changes:{notes:[...]}} (frames are addressable by id for update only). Prefer a real example (source) over an illustrative one (code).
